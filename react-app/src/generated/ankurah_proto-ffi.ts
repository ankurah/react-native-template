...
interface NativeModuleInterface {
  ...
  ubrn_uniffi_ankurah_proto_fn_clone_entityid(
    ptr: bigint,
    uniffi_out_err: UniffiRustCallStatus,
  ): bigint;
  ubrn_uniffi_ankurah_proto_fn_free_entityid(
    ptr: bigint,
    uniffi_out_err: UniffiRustCallStatus,
  ): void;
  ubrn_uniffi_ankurah_proto_fn_method_entityid_to_string(
    ptr: bigint,
    uniffi_out_err: UniffiRustCallStatus,
  ): Uint8Array;
  ubrn_uniffi_ankurah_proto_checksum_method_entityid_to_string(): number;
  ubrn_ffi_ankurah_proto_uniffi_contract_version(): number;
  ubrn_uniffi_internal_fn_method_entityid_ffi__bless_pointer(
    pointer: bigint,
    uniffi_out_err: UniffiRustCallStatus,
  ): UniffiRustArcPtr;
}

// Casting globalThis to any allows us to look for `NativeAnkurahProto`
// if it was added via JSI.
//
// We use a getter here rather than simply `globalThis.NativeAnkurahProto` so that
// if/when the startup sequence isn't just so, an empty value isn't inadvertantly cached.
const getter: () => NativeModuleInterface = () =>
  (globalThis as any).NativeAnkurahProto;
